...
Every published tag is immutable — `0.24.0` is always the same image — so an
...
docker run --rm -e DATABASE_URL="postgresql://…" \
  ghcr.io/rbrasier/wayfinder:0.24.0 migrate
```
...
sed -i 's/^WAYFINDER_VERSION=.*/WAYFINDER_VERSION=0.24.0/' .env
...
- **Read the release notes** for the version you are moving to, and for any you
  ...
  from `0.22.x` to `0.24.0` means you are taking `0.23.x`'s migrations too.

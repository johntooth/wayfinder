...
// Whether any administrator already exists. Split out from IAdminAccountCreator
// so callers that only ask the question — the startup setup-link emitter runs on
// the boot path — can be given an implementation that needs nothing but a
// database handle, rather than the whole auth provider.
export interface IAdminLookup {
  // Drives the public `adminExists` read, the no-admin redirect, and the
  // application-layer fast-fail.
...
export interface IAdminAccountCreator extends IAdminLookup {

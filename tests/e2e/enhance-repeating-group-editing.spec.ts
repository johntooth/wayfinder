 ...
 * We mock that single procedure to return a step whose output contains a group
 * field with items, then assert the browser renders the group as a table with
 * humanised column headers and per-item cells — exercising buildGroupTable and
 * the GroupCell renderer through the running app.
 *
...
// A single-procedure tRPC batch response, superjson-encoded (no special types,
// so the envelope is just { json }). ISO date strings survive `new Date(...)`.
const stepDataResponse = () => [
  {
    result: {
      data: {
        json: [
...
test.describe('enhance: repeating group items in Show Data', () => {
  test('a group renders as a table with humanised headers and per-item rows', async ({ page }) => {
    ...
    // Fulfil only the solo session.stepData query; let every other tRPC call
    // (including batched ones) hit the real backend.
    await page.route('**/api/trpc/**', async (route: Route) => {
      const pathname = new URL(route.request().url()).pathname;
      const procedures = pathname.slice(pathname.indexOf('/api/trpc/') + '/api/trpc/'.length);
      if (procedures === 'session.stepData') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify(stepDataResponse()),
        });
        return;
      }

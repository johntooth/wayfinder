...
test.describe("frontline fix this answer", () => {
  test.beforeEach(() => {
    test.skip(!process.env.E2E_SESSION_PATH, "Needs seeded knowledge-base session, which seedE2EFixtures does not create yet — see README 'Two reasons a spec is parked'.");
  });
...
test.describe("SME curation grid", () => {
  test.beforeEach(() => {
    test.skip(!process.env.E2E_SESSION_PATH, "Needs seeded knowledge-base chunks, which seedE2EFixtures does not create yet — see README 'Two reasons a spec is parked'.");
  });

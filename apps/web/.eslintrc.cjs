/* eslint-env node */
// Web app lint config. Extends the workspace TypeScript rules and layers the
...
//
// validate.sh check "web accessibility (jsx-a11y)" runs this config in
// isolation so a11y regressions fail CI even if the general lint is weakened.
module.exports = {
  root: true,
  ...
  settings: {
    "jsx-a11y": {
      // Map the project's component primitives to their underlying elements so
      // the linter checks them as the real interactive element.
      components: {
        Button: "button",
        Input: "input",
        Textarea: "textarea",
